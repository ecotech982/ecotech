...
export default function LoginModal() {
  ...
  const [mode, setMode] = useState<'signin' | 'signup' | 'select_google_account'>('signin');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      } else if (err.code === 'GOOGLE_PROMPT_REQUIRED') {
        // Open Google Account Picker view
        setMode('select_google_account');
        setError(null);
      } else {
        // Show Google account selection view so user can choose their Google email
        setMode('select_google_account');
        setError(null);
      }
    } finally {
  ...
  const handleConfirmGoogleAccount = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      await loginWithGoogle(googleEmail.trim(), googleName.trim() || googleEmail.split('@')[0]);
      closeLoginModal();
    } catch (err: any) {
      console.error(err);
      setError('Gagal masuk dengan akun Google ini. Silakan periksa kembali email Anda.');
    } finally {
      setIsSubmitting(false);
    }
  };
          ...
          <div className="bg-linear-to-r from-brand-blue/10 via-white to-brand-green/10 p-6 pb-5 relative border-b border-gray-100">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-xl font-extrabold text-gray-900 tracking-tight">
                {mode === 'select_google_account' 
                  ? 'Pilih Akun Google Anda'
                  : mode === 'signin' 
                  ? 'Masuk ke Akun ECOTECH' 
                  : 'Daftar Akun Baru'}
              </h3>
              <p className="text-xs text-gray-500 mt-1 max-w-xs">
                {mode === 'select_google_account'
                  ? 'Masukkan atau pilih alamat email Google yang ingin digunakan'
            ...
            {mode === 'select_google_account' ? (
              ...
              <form onSubmit={handleConfirmGoogleAccount} className="space-y-4">
                <div className="bg-blue-50/60 border border-blue-100 rounded-2xl p-4 flex items-center gap-3">
                  <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-xs flex-shrink-0">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-gray-900">Masuk dengan Google</h4>
                    <p className="text-xs text-gray-600">Silakan tentukan akun Google Anda di bawah ini</p>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-700 ml-1">Email Google / Gmail *</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-700 ml-1">Nama Tampilan Google (Opsional)</label>
                  <div className="relative">
                ...
                <div className="pt-2 space-y-2">
                  <button
                    ...
                    className="w-full py-3.5 px-4 bg-brand-blue hover:bg-brand-blue/90 text-white font-bold rounded-2xl shadow-md hover:shadow-lg transition-all duration-200 flex items-center justify-center gap-2 text-sm disabled:opacity-50"
                  >
                    {isSubmitting ? (
                      <>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-2xl transition-all text-xs"
                  >
                    Kembali ke Option Utama
                  </button>
                </div>
              </form>
            ) : (
